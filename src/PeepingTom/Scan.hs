...
updateMatchesHelper :: IO.RInterface -> [Match] -> IO.MemoryChunk -> IO [Match]
updateMatchesHelper _ [] _ = return []
updateMatchesHelper rinterface (current : rest) memory_chunk = do
    if (isInChunk current memory_chunk)
        then do
            ...
            tail <- updateMatchesHelper rinterface rest memory_chunk
            ...
            updateMatchesHelper rinterface (current : rest) new_chunk

updateMatches :: IO.RInterface -> ScanOptions -> [Match] -> IO [Match]
updateMatches _ _ [] = return []
updateMatches rinterface scopt matchs = do
    let chunk_size = soChunkSize scopt
    let initial = matchs !! 0
    let new_chunk_addr = mAddress initial
    chunk <- rinterface new_chunk_addr chunk_size
    updateMatchesHelper rinterface matchs chunk
...
updateStateS scopt state = do
    let pid = psPID state
    let stopsig = soSIGSTOP scopt
    let matchs = psMatches state
    let action = (\rinterface -> updateMatches rinterface scopt matchs) :: IO.RInterface -> IO [Match]
    matchs' <- IO.withRInterface pid stopsig action
    return $ state{psMatches = matchs'}
